...
class AuctionContol(signalControl.signalControl):
    def __init__(self, junctionData, minGreenTime=10., maxGreenTime=60., scanRange=250, auctionRate=30):
        super(AuctionContol, self).__init__()
        ...
        self.DCCstate = {'RELAXED': self.TGenCamMin,
                         'ACTIVE':0.2,
                         'RESRICTIVE': 0.25}
        ...
        # Bidding Params
        self.auctionRate = auctionRate  # dt
        self.bidAlpha = 0.01
        self.bidBeta = 0.001
    ...
    def process(self):
        ...
        super(AuctionContol, self).process(self.TIME_MS)
        ...
        for lane in self.activeLanes[self.lastStageIndex]:
            for vehID in self.CamRxData.keys():
                ...
                if (np.isclose(self.CamRxData[vehID]['h'], self.laneDetectionInfo[lane][0], atol=10)
                    ...
                    # Then append vehicle
                    vehicleIDs.append(vehID)

        vehicleIDs = list(np.unique(vehicleIDs))
        return vehicleIDs
    ...
    def BID1(self):
        bid = np.zeros_like(self.controlledLanes)
        for i, lane in enumerate(self.controlledLanes):
            bid[i] = sum([1 for x in self._getOncomingVehicles(laneID=lane)\
                          if self.CamRxData[x]['v'] < 0.5])
        return bid

    def BID2(self):
        bid = np.zeros_like(self.controlledLanes)
        for i, lane in enumerate(self.controlledLanes):
            vehicleIDs = self._getOncomingVehicles(laneID=lane)
            bid[i] = 0 
            for vID in vehicleIDs:
                V = self.CamRxData[vID]['v']
                vehPos = np.array(self.CamRxData[ID]['pos'])
                X = np.linalg.norm(vehPosition - self.jcnPosition)
                bid[i] += 1 - self.bidAlpha*V - self.bidBeta*X
        return bid
