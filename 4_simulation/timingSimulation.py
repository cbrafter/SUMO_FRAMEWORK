...
def simulation(configList, GUIbool=False, weightArray=np.ones(7, dtype=float),
    ...
    try:
        ...
        timingFilename = exportPath+'timinginfo_R{:03d}_CVP{:03d}.csv'.format(seed, int(CVP*100))
        ...
        while simActive:
            ...
            if 'CDOTS' in tlLogic:
                if not simTime%(15*oneMinute):
                    timeData = np.zeros(100)
                    tstart = time.time()
                    for i in range(1,101):
                        for controller in controllerList[:]:
                            controller.process(time=simTime, stopCounter=stopCounter,
                                               emissionCounter=emissionCounter)
                        timeData[i-1] = time.time()-tstart
                    timingResults.append(timeData)
            else:
                if not simTime%(15*oneMinute):
                    timeData = np.zeros(100)
                    tstart = time.time()
        ...
        connector.disconnect()
        timingResults = np.array(timingResults)
        np.savetxt(timingFilename, timingResults, delimiter=',')
