...
lineStyle = {'CDOTS-1111001': 'vC0',
             'CDOTS-1101001': '^C1',             
             'CDOTS-1101000': 'vC2',
             'CDOTSslow-1111001': 'vC0',
             'CDOTSslow-1101001': '*C1',
             'CDOTSslow-1101000': 'oC2'}
...
pdfFileName = 'CDOTS_COMP_delay{}.pdf'.format(pedString)
figuresPDF = PdfPages(pdfFileName)
print('Writing: '+pdfFileName)

# only store the cols we need to save memory
selectCols = ['controller','model','run','cvp','routeLength','duration']
...
for model in models:
    ...
    for controller in controllers[-2:]:
        ...
        # subset plot data
            ...
            data['duration'] = data['duration']/(data['routeLength']*0.001)    
            # if no data in this set continue
            if data.empty:
                print('{} {} *NULL*'.format(model, controller))
                continue
            else:
                data = data.groupby('cvp').duration
            ...
            print(model, AA, controller, meanDelay[-1])
