...
def parallelParser(x):
    try:
        ...
        resultFolder = '/hardmem/results_HVAF/'+controller+'/'+model+'/'
        # Storage for Results
        ...
        # Use try except so that we can identify corrupted data and rerun without
...
configs = list(itertools.product(models, tlControllers[2:3]))
