...
def likertMap(x):
    return rangeMap(x, [1, 5], [-100, 100])


def likertMap10(x):
    return rangeMap(x, [1, 10], [-100, 100])
...
# Education
objects = ['HIGHSCHOOL', 'DIPLOMA', 'BACHELORS',
           'MASTERS', 'DOCTORATE', 'OTHER']
labels = ['Secondary', 'Diploma', 'Bachelor\'s',
...
values = [data.socialMediaUsage[data.socialMediaUsage == x].count()
          * scale for x in objects]
...
for i in range(1, 11):
    objects.append(data[data.socialMediaAcceptance ==
                        i].socialMediaAcceptance.count())
...
print('Social Media Sentiment Score',
      data.socialMediaAcceptance.apply(likertMap10).mean())

# Social Media Acceptance 1-5: down sample the above likert
objects = []
data['socialMediaAcceptanceDS'] = (
    (data.socialMediaAcceptance / 2.0) + 0.1).apply(np.round).apply(int)
scale = 100.0 / float(data.socialMediaAcceptanceDS.count())
for i in range(1, 6):
    objects.append(data[data.socialMediaAcceptanceDS ==
                        i].socialMediaAcceptanceDS.count())
x_pos = np.arange(1, len(objects) + 1)
fig = plt.figure()
plt.bar(x_pos, objects, align='center', width=0.5)
plt.xticks(x_pos, fontsize=ticksize, fontweight='bold')
...
plt.grid(axis='y', linestyle='--', linewidth=1, alpha=0.5)
savePDF(figuresPDF, fig)
print('Social Media Mean Score', data.socialMediaAcceptanceDS.mean())
print('Social Media Median Score', data.socialMediaAcceptanceDS.median())
print('Social Media Std Score', data.socialMediaAcceptanceDS.std())
print('Social Media Sentiment Score',
      data.socialMediaAcceptanceDS.apply(likertMap).mean())
...
# Car Access
total = data.hasCarAccess.count()
yes = 100 * data.hasCarAccess[data.hasCarAccess ==
                              'YES'].count() / float(total)
...
values = [data.rideShareUsage[data.rideShareUsage == x].count() *
          scale for x in objects]
...
values = [data.pedCycleFreq[data.pedCycleFreq == x].count() *
          scale for x in objects]
...
plt.xticks(index, ('Car', 'EV', 'Bus', 'M/cycle', 'LGV', 'HGV',
                   'None'), fontsize=ticksize, fontweight='bold', rotation=90)
plt.legend(loc='upper center', bbox_to_anchor=(
    0.5, 1.25), ncol=3, fontsize=axsize - 4)
savePDF(figuresPDF, fig)
...
# Map Usage
data.mapUsage.fillna('NA', inplace=True)
objects = ['WEEKLY_MULTI', 'WEEKLY_ONCE', 'MONTHLY', 'RARELY', 'NA']
labels = ['Daily', 'Weekly', 'Monthly', 'Rarely', 'NA']
x_pos = np.arange(len(objects))
scale = 100.0 / float(data.mapUsage.count())
values = [data.mapUsage[data.mapUsage == x].count() *
          scale for x in objects]
...
fig = plt.figure()
plt.bar(x_pos, values, align='center', width=0.5)
plt.xticks(x_pos, ['Yes', 'No', 'No Preference'],
...
print('UTM Sentiment Score', data.UTMAcceptance.apply(likertMap10).mean())

objects = []
data['UTMAcceptanceDS'] = (
    (data.UTMAcceptance / 2.0) + 0.1).apply(np.round).apply(int)
...
plt.grid(axis='y', linestyle='--', linewidth=1, alpha=0.5)
savePDF(figuresPDF, fig)
print('UTM Mean Score', data.UTMAcceptanceDS.mean())
print('UTM Median Score', data.UTMAcceptanceDS.median())
print('UTM Std Score', data.UTMAcceptanceDS.std())
print('UTM Sentiment Score', data.UTMAcceptanceDS.apply(likertMap).mean())


# Preferred Data Sharing Method
data.deliveryMode.fillna('NA', inplace=True)
objects = ['APP', 'BUILTIN', 'THIRDPARTY', 'OTHER', 'NOSHARE']
labels = ['App', 'Built-in', '3rd Party', 'Other']
x_pos = np.arange(len(objects))
scale = 100.0 / float(data.deliveryMode.count())
values = [data.deliveryMode[data.deliveryMode.apply(
    lambda y: x in y)].count() * scale for x in objects]
...
plt.xticks(index + bar_width, ('Bus', 'Em. Svc.', 'Shared', 'EV', 'Trade'),
           fontsize=ticksize, fontweight='bold',
           rotation=90)
plt.legend(loc='upper center', bbox_to_anchor=(
    0.5, 1.25), ncol=3, fontsize=axsize - 4)
...
def cmapFn(x):
    if x >= 0:
        return cmapPos(abs(1 - x / 100.0))
    else:
        return cmapNeg(1 - abs(x / 100.0))


barColors = list(map(cmapFn, 1.0 * values))  # scale values to max

fig = plt.figure(figsize=(7, 4))
cmapCB = matplotlib.colors.ListedColormap(
    [cmapFn(x) for x in np.linspace(-100, 100, 1000)])
...
plt.bar(x_pos, values, align='center', width=0.5, color=barColors)
plt.xticks(x_pos, labels, fontsize=ticksize, fontweight='bold', rotation=90)
plt.ylim([-30, 60])
plt.yticks(np.arange(-30, 61, 10), fontsize=ticksize, fontweight='bold')
ax = plt.gca()
ax.yaxis.set_minor_locator(tck.MultipleLocator(5))
...
x_pos = np.arange(len(objects))
fig = plt.figure()
df = data.loc[:, objects[::-1]]
sns.boxplot(data=df, orient='h', whis=[5, 95])
plt.yticks(x_pos, labels[::-1], fontsize=ticksize, fontweight='bold')
plt.xticks(np.arange(1, 5.1, 1, dtype=int),
           fontsize=ticksize, fontweight='bold')
ax = plt.gca()
ax.set_axisbelow(True)
plt.grid(axis='x', linestyle='--', linewidth=1, alpha=0.5, which='both')
plt.title('Data Sharing Likert Boxplots', fontsize=tsize)
plt.xlabel('Likert Value', fontsize=axsize)
...
for obj in objects:
    N = float(data[obj].count())
    ONE.append(data[obj][data[obj] == 1].count() / N)
    TWO.append(data[obj][data[obj] == 2].count() / N)
    THREE.append(data[obj][data[obj] == 3].count() / N)
    FOUR.append(data[obj][data[obj] == 4].count() / N)
    FIVE.append(data[obj][data[obj] == 5].count() / N)

ONE = 100 * np.array(ONE)
TWO = 100 * np.array(TWO)
THREE = 100 * np.array(THREE)
FOUR = 100 * np.array(FOUR)
FIVE = 100 * np.array(FIVE)

bar_width = 0.7
rects1 = plt.barh(index, ONE, bar_width,
                  color=cmapFn(-70), label='1', linewidth=0.5, edgecolor='k')

rects2 = plt.barh(index, TWO, bar_width,
                  color=cmapFn(-30), label='2', left=ONE,
                  linewidth=0.5, edgecolor='k')

rects3 = plt.barh(index, THREE, bar_width,
                  color=cmapFn(5), label='3', left=ONE + TWO,
                  linewidth=0.5, edgecolor='k')

rects5 = plt.barh(index, FOUR, bar_width,
                  color=cmapFn(30), label='4', left=ONE + TWO + THREE,
                  linewidth=0.5, edgecolor='k')

rects5 = plt.barh(index, FIVE, bar_width,
                  color=cmapFn(70), label='5', left=ONE + TWO + THREE + FOUR,
                  linewidth=0.5, edgecolor='k')

plt.xlim([0, 100])
plt.xlabel('Likert Value Percent Share', fontsize=axsize)
plt.yticks(x_pos, labels, fontsize=ticksize, fontweight='bold')
...
# ax.set_axisbelow(True)
plt.grid(axis='x', linestyle='--', linewidth=1, alpha=0.5, which='both')
plt.title('Data Sharing Distribution', fontsize=tsize)
# plt.legend(loc='upper right', bbox_to_anchor=(1.21, 1.03),
#            ncol=1, fontsize=axsize-4)
plt.legend(loc='lower center', bbox_to_anchor=(
    0.5, -0.3), ncol=5, fontsize=axsize - 4)
savePDF(figuresPDF, fig)

# Social media UTMS sharing comparison
fig = plt.figure()
SM = []
UTMS = []

scaleSM = 100.0 / float(data.socialMediaAcceptance.count())
scaleUTMS = 100.0 / float(data.UTMAcceptance.count())
for i in range(1, 11):
    SM.append(data[data.socialMediaAcceptance == i].socialMediaAcceptance.count())
    UTMS.append(data[data.UTMAcceptance == i].UTMAcceptance.count())

# create plot
fig, ax = plt.subplots(figsize=[8,4])
index = np.arange(1,11,dtype=int)
bar_width = 0.33

rects1 = plt.bar(index, SM, bar_width,
                 color='C0',
                 label='Social Media')

rects2 = plt.bar(index + bar_width, UTMS, bar_width,
                 color='C1',
                 label='Traffic Management')

plt.ylim([0, 26])
plt.yticks(np.arange(0, 27, 2), fontsize=ticksize)
plt.xticks(index + 0.5*bar_width, np.arange(1,11,1),
           fontsize=ticksize)
ax = plt.gca()
...
plt.grid(axis='y', linestyle=':', linewidth=1, alpha=0.5, which='minor')
plt.ylabel('Percentage', fontsize=axsize - 2)
plt.title('User Willingness to Share Data Based on Application', fontsize=tsize - 4)
plt.legend(loc='upper center', bbox_to_anchor=(0.5, 1.25),
           ncol=3, fontsize=axsize - 4)
savePDF(figuresPDF, fig)
