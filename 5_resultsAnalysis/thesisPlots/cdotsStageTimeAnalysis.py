...
def pathParse(path):
    base = '/hardmem/results/'
    if 'DOTS' not in path:
        regex = '(.+?)/(.+?)/stageinfo_R(.+?)_CVP(.+?).csv'
    else:
        regex = '(.+?)/(.+?)/.+?/stageinfo_R(.+?)_CVP(.+?).csv'
    match = re.match(base+regex, path)
...
stagefiles = glob('/hardmem/results/**/**/stageinfo*.csv') +\
             glob('/hardmem/results/**/**/**/stageinfo*.csv')
data = []
for fname in stagefiles:
    ...
    if '_ped' in controller or 'TRANSYT' in controller:
        ...
        df['start'] = df.simTime.values - \
            df.stageDuration.values  # when the occurance started
        ...
        for junc in df.junction.unique():
            ...
            for stage in subset1.stageID.unique():
                ...
                # interval between when the last occurance stopped and the
                # current occurance offset to create time shift
                interval = subset2.start.values[1:] - subset2.stop.values[:-1]
                interval = np.array([0.0] + list(interval))
                df.loc[(df.junction == junc) &
                       (df.stageID == stage), 'interval'] = interval
        data.append(df)
...
controllers = ['GPSVA_ped', 'CDOTS_ped']
...
pdfFileName = 'CDOTS_stageTimes.pdf'
figuresPDF = PdfPages(pdfFileName)
print('Writing: ' + pdfFileName)

bins = np.arange(0, 301, 10, dtype=int)
    ...
    for cvp in [0]:
        for controller in controllers[:1]:
            ...
            for i, model in enumerate(models):
                ...
                plt.hist(df.interval, histtype='step', bins=bins,
                         label=labels[i], linewidth=1.5)
            ctrlName = ctrlMap[controller.split('_')[0]]
            titleStr = 'J{}: TRANSYT stage interval histogram'.format(
                junc[-1], ctrlName)
            plt.title(titleStr, fontsize=tsize, fontweight='bold')
            plt.xlabel('Stage interval [s]',
                       fontsize=axsize, fontweight='bold')
            plt.ylabel('Frequency', fontsize=axsize, fontweight='bold')
            plt.gca().xaxis.set_ticks(np.arange(0, 301, 50, dtype=int))
            plt.gca().xaxis.set_ticks(np.arange(0, 301, 10, dtype=int),
                                      minor=True)
        ...
        for controller in controllers:
            ...
            for i, model in enumerate(models):
                ...
                plt.hist(df.interval, histtype='step', bins=bins,
                         label=labels[i], linewidth=1.5)
            ctrlName = ctrlMap[controller.split('_')[0]]
            titleStr = 'J{}: {} stage interval histogram'.format(
                junc[-1], ctrlName)
            plt.title(titleStr, fontsize=tsize, fontweight='bold')
            plt.xlabel('Stage interval [s]',
                       fontsize=axsize, fontweight='bold')
...
#sample = data[(data.junction=='junc9')&(data.controller=='CDOTS_ped')&(data.cvp==100)&(data.model=='sellyOak_hi')&(data.interval>120)]
# sample.groupby('stageID').count()

# sample = data[(data.junction=='junc9')&(data.controller=='CDOTS_ped')&(data.cvp==100)&(data.model=='sellyOak_hi')&(data.interval>120)].simTime/3600
# plt.hist(sample, histtype='step', bins=np.arange(25, dtype=int));
